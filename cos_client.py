...
class CosS3Client(object):
    ...
    def put_object_lock(self, Bucket, ObjectLockConfiguration={}, **kwargs):
        lst = [  # 类型为list的标签
            '<Rule>',
            '</Rule>']
        xml_config = format_xml(data=ObjectLockConfiguration, root='ObjectLockConfiguration', lst=lst)
        ...
        params = {'object-lock': ''}
        url = self._conf.uri(bucket=Bucket)
        logger.info("put object lock, url=:{url} ,headers=:{headers}".format(
            url=url,
            headers=headers))
        rt = self.send_request(
            method='PUT',
    ...
    def get_object_lock(self, Bucket, **kwargs):
        headers = mapped(kwargs)
        params = {'object-lock': ''}
        url = self._conf.uri(bucket=Bucket)
        logger.info("get object lock, url=:{url} ,headers=:{headers}".format(
            url=url,
            headers=headers))
        rt = self.send_request(
            method='GET',
